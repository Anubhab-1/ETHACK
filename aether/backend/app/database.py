    ...
    if _is_postgres:
        with engine.connect() as conn:
            try:
                conn.execute(
                    text(
                        "SELECT create_hypertable('readings', 'measured_at', if_not_exists => TRUE);"
                    )
                )
                conn.execute(
                    text(
                ...
                conn.commit()
                logger.info(
                    "TimescaleDB hypertables, compression, and retention policy configured."
                )
            except Exception as e:
                logger.warning(
                    "Non-critical: TimescaleDB hypertable setup failed: %s", e
                )
