...
try:
    ...
    logger.info("Kafka library (kafka-python) not installed. Falling back to default polling services.")
...
class KafkaStreamManager:
    ...
    def __init__(self, bootstrap_servers: str = "localhost:9092"):
        ...
        if KAFKA_AVAILABLE:
            try:
                ...
                logger.warning(f"Failed to connect Kafka Producer to {self.bootstrap_servers}: {e}")
                self.producer = None
    ...
    def publish_event(self, topic_key: str, data: Dict[str, Any]) -> bool:
        ...
        if KAFKA_AVAILABLE and self.producer:
            try:
                ...
                logger.info(f"Published event to topic '{topic}': {list(data.keys())[:3]}")
                return True
            except Exception as e:
    ...
    def register_consumer(self, topic_key: str, consumer_group: str, callback: Callable[[Dict[str, Any]], None]):
        """
        ...
        logger.info(f"Registering consumer for topic '{topic}' on group '{consumer_group}'...")
        if KAFKA_AVAILABLE:
            try:
