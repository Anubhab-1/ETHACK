...
class PredictiveRiskScorer:
    ...
    def predict(self, source_features: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            X = pd.DataFrame([{k: source_features.get(k, 0) for k in self.FEATURE_COLUMNS}])
...
def generate_mock_sources_data(ward_id: int) -> List[Dict[str, Any]]:
    ...
    sources = []
    names = ["Brick Kiln Co.", "Sreeram Iron Foundry", "Bishnu Chemical Processing", "Kolkata Allied Metals", "Hooghly Paper Corp"]
    for i in range(n_sources):
        source_id = f"SRC-{ward_id:03d}-{i+1:02d}"
        sources.append({
            "id": source_id,
            "name": names[i % len(names)],
            "type": rng.choice(["Foundry", "Chemical", "Brick_Kiln", "Metallurgy", "Paper_Mill"]),
            "ward_id": ward_id,
