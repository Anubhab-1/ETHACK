...
class AetherKnowledgeGraph:
    ...
    def seed(self, db=None, city: str = "Kolkata"):
        ...
        for ward in wards:
            ...
            for i in range(n_industries):
                ...
                industry_type = rng.choice(["Foundry", "Chemical", "Textile", "Brick_Kiln", "Power_Plant"])
                ...
                self._add_edge(
                    stack_id, f"WARD-{ward.id}", "EMITS_INTO",
                    {"pollutant": "PM2.5", "rate": round(pm_emission, 1), "distance_km": round(rng.uniform(0.1, 3.0), 2)}
                )
                ...
                for v in range(violation_count):
                    ...
                    self._add_edge(industry_id, violation_id, "HAS_VIOLATION", {"severity": severity})
                    ...
                    if severity in ["high", "critical"]:
                        ...
                        self._add_node(
                            ...
                            props={
                                ...
                                "outcome_duration_hours": rng.randint(24, 168) if ate < -30 else None,
                            }
                        )
    ...
    def get_industry_risk_graph(self, ward_id: int) -> Dict[str, Any]:
        ...
        for ind_id in industries_in_ward:
            violations = [
                ...
                if e["rel"] == "HAS_VIOLATION" and e["src"] == ind_id and e["dst"] in self._nodes
            ]
        ...
        return {
            ...
            "summary": {
                ...
                "total_violations": sum(len(v) for v in violations_by_industry.values()),
                "top_risk_industries": [
                    {"id": n["id"], "name": n.get("name"), "risk_score": n.get("risk_score", 0),
    ...
    def get_intervention_outcomes(self, ward_id: int, action_type: Optional[str] = None) -> List[Dict]:
        """
        ...
        enforcement_nodes = [
            ...
            if v.get("type") == "EnforcementAction" and
            v.get("outcome_aqi_drop") is not None
        ]

        if action_type:
            enforcement_nodes = [n for n in enforcement_nodes if n.get("type") == action_type]
    ...
    def get_pagerank_polluters(self, city: str) -> List[Dict]:
        ...
        if not _NX_AVAILABLE or self._graph is None:
            ...
            for node_id, degree in sorted(out_degrees.items(), key=lambda x: -x[1])[:5]:
                ...
                industry = next(
                    (self._nodes[e["src"]] for e in self._edges
                     if e["rel"] == "OPERATES" and e["dst"] == node_id and e["src"] in self._nodes),
                    None
                )
                if industry:
                    results.append({
        ...
        try:
            ...
            for stack_id, score in top_stacks:
                industry = next(
                    (self._nodes[e["src"]] for e in self._edges
                     if e["rel"] == "OPERATES" and e["dst"] == stack_id and e["src"] in self._nodes),
                    None
                )
                if industry:
                    results.append({
    ...
    def get_violation_clusters(self) -> List[Dict]:
        ...
        for cell, members in clusters.items():
            if len(members) >= 2:
                total_violations = sum(m.get("historical_violations", 0) for m in members)
                results.append({
                    "cluster_id": cell,
                    "member_count": len(members),
                    "total_violations": total_violations,
                    "avg_risk_score": round(sum(m.get("risk_score", 0) for m in members) / len(members), 2),
                    "members": [{"id": m["id"], "name": m.get("name"), "violations": m.get("historical_violations")} for m in members],
