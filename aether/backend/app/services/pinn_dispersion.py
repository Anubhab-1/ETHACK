...
try:
    ...
    logger.info(
        "PyTorch not available or failed to import. PINN dispersion model will use analytical Gaussian plume fallback."
    )
...
class DispersionPINN(nn.Module):
    ...
    def forward(self, xyt_sw):
        ...
        if not TORCH_AVAILABLE:
            raise RuntimeError(
                "PyTorch is not available. Cannot run forward pass on PINN."
            )
    ...
    def physics_loss(self, xyt_sw, D: float = 0.15, k: float = 0.05):
        ...
        grads = torch.autograd.grad(
            c, xyt_sw, grad_outputs=torch.ones_like(c), create_graph=True
        )[0]
        c_x = grads[:, 0:1]
        c_y = grads[:, 1:2]
        c_t = grads[:, 2:3]
        ...
        c_xx = torch.autograd.grad(
            c_x, xyt_sw, grad_outputs=torch.ones_like(c_x), create_graph=True
        )[0][:, 0:1]
        c_yy = torch.autograd.grad(
            c_y, xyt_sw, grad_outputs=torch.ones_like(c_y), create_graph=True
        )[0][:, 1:2]
...
def get_trained_pinn() -> Optional[DispersionPINN]:
    ...
    logger.info(
        "Initializing and training Physics-Informed Neural Network (PINN) dispersion model on CPU..."
    )
    ...
    if TORCH_AVAILABLE:
        try:
            ...
            if model is not None:
                ...
                inputs = torch.tensor(
                    [[x_wind, y_wind, hours, strength, wind_speed]], dtype=torch.float32
                )
                model.eval()
                with torch.no_grad():
