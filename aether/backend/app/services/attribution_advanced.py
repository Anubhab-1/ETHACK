...
from app.models import Ward, Reading, Station, Weather
...
try:
    ...
    logger.warning("scikit-learn NMF not available. Using local multiplicative update solver.")
...
class PMFSourceApportionment:
    ...
    def fit(self, speciation_data: pd.DataFrame, n_bootstrap: int = 50):
        ...
        if SKLEARN_AVAILABLE:
            try:
                ...
                logger.warning(f"sklearn NMF fitting failed: {e}. Falling back to local solver.")
        ...
        for b in range(n_bootstrap):
            ...
            if SKLEARN_AVAILABLE:
                try:
                    model_b = NMF(n_components=self.n_sources, max_iter=300, 
                                  random_state=self.random_state + b, solver='mu')
        ...
        for i, profile in enumerate(F_base):
            ...
            dominant_species = speciation_data.columns[dominant_idx] if dominant_idx < len(speciation_data.columns) else "Unknown"
    ...
    def _fit_local(self, X: np.ndarray, max_iter: int = 300) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def get_latest_contributions(self, speciation_cols: List[str]) -> Dict[str, Any]:
        ...
        for i, (name, contrib) in enumerate(zip(self.source_names_, latest)):
            bootstrap_samples = self.G_bootstrap_[:, i] if self.G_bootstrap_ is not None else np.array([contrib])
            boot_total = max(1e-9, bootstrap_samples.mean())
            
            ci_lower = np.percentile(bootstrap_samples, 2.5) if len(bootstrap_samples) > 1 else contrib
            ci_upper = np.percentile(bootstrap_samples, 97.5) if len(bootstrap_samples) > 1 else contrib
            
            # Find dominant species in profile
            profile = self.source_profiles_[i]
            dom_idx = int(np.argmax(profile))
            dominant_species = speciation_cols[dom_idx] if dom_idx < len(speciation_cols) else "Unknown"
...
def fetch_speciation_data_db(ward: Ward, db: Session) -> pd.DataFrame:
    ...
    stations = db.query(Station).filter(Station.city == ward.city, Station.active == True).limit(3).all()
    if not stations:
    ...
    ).order_by(Reading.measured_at.desc()).limit(50).all()
...
def ensemble_attribution(pmf_result: Dict, inventory: Dict) -> Dict:
    ...
    if not pmf_result.get('contributions'):
        categories = {
            'Secondary Sulfate (Coal/Industrial)': inventory['industrial'] + inventory['secondary_aerosols'] * 0.4,
            'Biomass Burning / Vehicles': inventory['biomass_burning'] + inventory['traffic'] * 0.3,
            'Mineral Dust / Construction': inventory['construction'],
            'Secondary Nitrate (Vehicles)': inventory['traffic'] * 0.7 + inventory['secondary_aerosols'] * 0.6,
        ...
        inv_weights = {
            ...
            'Biomass Burning / Vehicles': (inventory['biomass_burning'] + inventory['traffic'] * 0.4) / inv_total,
