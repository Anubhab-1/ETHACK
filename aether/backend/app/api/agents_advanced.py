...
def log_deliberation_to_db(result: dict):
    ...
    try:
        ...
        db.commit()
        logger.info(
            f"Deliberation audit successfully persisted to DB for Ward {result['ward_id']}"
        )
...
async def run_advanced_deliberation(
    ward_id: str, background_tasks: BackgroundTasks, db: Session = Depends(get_db)
):
    """
...
async def get_deliberation_history(
    ward_id: str, limit: int = 10, db: Session = Depends(get_db)
):
    """
    ...
    logs = (
        db.query(DeliberationLog)
        .filter(DeliberationLog.ward_id == ward.id)
        .order_by(DeliberationLog.created_at.desc())
        .limit(limit)
        .all()
    )
    ...
    if not history:
        ...
        history = [
            {
                "timestamp": (
                    now - __import__("datetime").timedelta(days=int(i))
                ).isoformat(),
