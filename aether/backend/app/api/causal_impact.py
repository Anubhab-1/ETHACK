...
async def get_city_causal_history(city: str = Query("Kolkata"), db: Session = Depends(get_db)):
    ...
    try:
        ...
        actions = (
            ...
            .filter(EnforcementAction.city == city, EnforcementAction.status == "resolved")
        ...
        for action, ward in actions:
            try:
                ...
                results.append({
                    "action_id": action.id,
                    "intervention": action.target_type.replace("_", " ").capitalize(),
                    "ward": ward.name,
