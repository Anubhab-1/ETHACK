...
from app.models import Ward, Station, Reading, Weather
from app.models.st_gcn import AetherSTGCN, build_wind_aligned_graph, TORCH_AVAILABLE
...
async def get_stations_for_ward(ward: Ward, db: Session) -> List[Station]:
    ...
    stations = db.query(Station).filter(Station.ward_id == ward.id, Station.active == True).all()
    if not stations:
...
async def fallback_xgboost_forecast(ward: Ward, db: Session, hours: int) -> Dict[str, Any]:
    ...
    from app.services.forecaster import predict_aqi, get_current_aqi_for_ward
    ...
    if TORCH_AVAILABLE:
        try:
            ...
            with torch.no_grad():
                ...
                pred_out = model(x, edge_index, edge_weight).numpy()
