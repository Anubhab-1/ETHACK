...
def ensure_readings_exist(city: str, db: Session):
    ...
    has_readings = (
        db.query(Reading).filter(Reading.station_id.in_(station_ids)).first()
        is not None
    )
    if not has_readings:
        log.info(
            f"No readings found in DB for {city} stations. Fetching live data synchronously..."
        )
        try:
            ...
            log.info(
                "WAQI not configured or failed. Falling back to legacy CPCB fetcher."
            )
...
def get_aqi_heatmap(city: str = Query("Kolkata"), db: Session = Depends(get_db)):
    ...
    station_map = {
        st.id: st for st in db.query(Station).filter(Station.city == city).all()
    }
    station_points = [
        (station_map[r.station_id].lat, station_map[r.station_id].lon, r.aqi)
        for r in latest_data
        if r.station_id in station_map and r.aqi
    ]
